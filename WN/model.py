...
# -----------------------------------------------------------------------------
# DecreCost ------------------------------------------------------------------------
def decremargincost(pos, neg, marge=1.0):
    out = pos - neg + marge
    return T.sum(out * (out > 0)), out > 0
# -----------------------------------------------------------------------------
...
def ForwardFn(fnsim, embeddings, leftop, rightop, marge=1.0):
    ...
    return theano.function([inpl, inpr, inpo,
                           ...
                           on_unused_input='ignore')
def DecreTrainFn1Member(fnsim, embeddings, leftop, rightop, marge=1.0, rel=True, postion = ''):
    """
    This function returns a theano function to perform a training iteration,
    ...
    costl, outl = decremargincost(simi, similn, marge)
    costr, outr = decremargincost(simi, simirn, marge)
    # cost = costl + costr
    ...
    if rel:
        ...
        costo, outo = decremargincost(simi, simion, marge)
        # cost += costo
        # out = T.concatenate([out, outo])
        list_in += [inpon]
        if postion == 'lhs':
            cost = costr + costo
            out = T.concatenate([outr, outo])
        elif postion == 'rhs':
            cost = costl + costo
            out = T.concatenate([outl, outo])
    if postion == 'lhs':
        cost = costr
        out = outr
    elif postion == 'rhs':
...
def RankingScoreIdx(sl, sr, idxl, idxr, idxo):
    ...
    for l, o, r in zip(idxl, idxo, idxr):
        ...
        top10_left.append(np.argsort((sl(r, o)[0]).flatten())[::-1][:9].tolist())
        errr += [np.argsort(np.argsort((
            sr(l, o)[0]).flatten())[::-1]).flatten()[r] + 1]
        top10_right.append(np.argsort((sr(l, o)[0]).flatten())[::-1][:9].tolist())
    return errl, errr, top10_left, top10_right
